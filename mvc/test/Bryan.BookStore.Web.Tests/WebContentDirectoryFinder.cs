...
public static class WebContentDirectoryFinder
{
    public static string? CalculateContentRootFolder()
    {
        ...
        {
            while (!DirectoryContains(directoryInfo.FullName, "Bryan.BookStore.Web.csproj", SearchOption.AllDirectories))
            {
            ...
            var webProject = Directory.GetFiles(directoryInfo.FullName, string.Empty, SearchOption.AllDirectories)
                .First(filePath => string.Equals(Path.GetFileName(filePath), "Bryan.BookStore.Web.csproj"));

            return Path.GetDirectoryName(webProject);
        }

        while (!DirectoryContains(directoryInfo.FullName, "Bryan.BookStore.sln"))
        {
            directoryInfo = directoryInfo.Parent ?? throw new Exception("Could not find content root folder!");
        }

        var webFolder = Path.Combine(directoryInfo.FullName, $"src{Path.DirectorySeparatorChar}Bryan.BookStore.Web");
